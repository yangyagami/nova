...
export const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  listProjects: async () => {
    ...
    try {
      ...
      const projects = await db.select<Project[]>(
        "SELECT id, title, subgenre, premise, target_words as targetWords, target_chapters as targetChapters, words_per_chapter as wordsPerChapter, status, created_at as createdAt, updated_at as updatedAt FROM projects ORDER BY updated_at DESC"
      );
  ...
  createProject: async (input: CreateProjectInput) => {
    ...
    const db = await getDb();
    await db.execute(
      `INSERT INTO projects (id, title, subgenre, premise, target_words, target_chapters, words_per_chapter, status, created_at, updated_at)
  ...
  getProject: async (id: string) => {
    ...
    try {
      const db = await getDb();
      const projects = await db.select<Project[]>(
        "SELECT id, title, subgenre, premise, target_words as targetWords, target_chapters as targetChapters, words_per_chapter as wordsPerChapter, status, created_at as createdAt, updated_at as updatedAt FROM projects WHERE id = $1",
        [id]
      );
  ...
  updateProject: async (id: string, input: CreateProjectInput) => {
    try {
      ...
      const timestamp = now();
      await db.execute(
        `UPDATE projects SET title = $1, subgenre = $2, premise = $3, target_words = $4, target_chapters = $5, words_per_chapter = $6, updated_at = $7 WHERE id = $8`,
        [
          input.title,
          input.subgenre,
          input.premise,
